#!/bin/bash
# Consultancy — Qwen3-30B-A3B-Instruct-2507 (no-think natively)
# 2 versions × 3 seeds × G=8 = 6 runs total
#
# v2: normalize_advantages_by_length=True, grad_clip=0.3
# v3: normalize_advantages_by_length=True, grad_clip=0.0
#
# Launch: bash scripts/launch_consultancy_q3instruct.sh
# Single: bash scripts/launch_consultancy_q3instruct.sh v2 42
#
# Monitor: uv run python -m scripts.spot_check_rlvr logs/consultancy-q3instruct/* --watch

set -euo pipefail

LOG_DIR="logs/consultancy-q3instruct"
mkdir -p "$LOG_DIR"

COMMON=(
  model_name=Qwen/Qwen3-30B-A3B-Instruct-2507
  renderer_name=qwen3_instruct
  dataset=consultancy_gpqa
  batch_size=64
  group_size=8
  max_tokens=8192
  n_batches=50
  learning_rate=1e-5
  temperature=1.0
  top_p=0.8
  top_k=20
  eval_temperature=0.7
  eval_top_p=0.8
  eval_top_k=20
  loss_fn=ppo
  clip_ratio_upper=0.28
  advantage_scheme=maxrl
  kl_penalty_coef=0.0
  format_coef=0.10
  eos_coef=0.10
  eval_every=5
  save_every=5
  wandb_project=consultancy-gpqa-q3instruct
  behavior_if_log_dir_exists=resume
)

IDX=0
run_one() {
  local VERSION=$1 SEED=$2
  local NAME="consult-q3-${VERSION}-g8-s${SEED}"

  # Version-specific overrides
  local EXTRA=()
  case "$VERSION" in
    v2) EXTRA=(normalize_advantages_by_length=True  grad_clip_norm=0.3) ;;
    v3) EXTRA=(normalize_advantages_by_length=True  grad_clip_norm=0.0) ;;
    *)  echo "Unknown version: $VERSION"; exit 1 ;;
  esac

  local DELAY=$((IDX * 90))
  echo "=== Launching ${NAME} (delay=${DELAY}s) ==="
  uv run --env-file .env python -m tinker_cookbook.recipes.consultancy.train \
    "${COMMON[@]}" \
    "${EXTRA[@]}" \
    seed="${SEED}" \
    log_path="${LOG_DIR}/${NAME}" \
    wandb_name="${NAME}" \
    initial_delay_s="${DELAY}" \
    > "${LOG_DIR}/${NAME}.stdout.log" 2>&1 &
  echo "  PID: $!  (log: ${LOG_DIR}/${NAME}.stdout.log)"
  IDX=$((IDX + 1))
}

# Single run mode: bash launch_consultancy_q3instruct.sh v2 42
if [[ $# -eq 2 ]]; then
  run_one "$1" "$2"
  wait
  exit 0
fi

# Full matrix: 2 versions × 3 seeds = 6 runs
PIDS=()
for VERSION in v2 v3; do
  for SEED in 42 137 7; do
    run_one "$VERSION" "$SEED"
    PIDS+=($!)
  done
done

echo ""
echo "=== All 6 launched ==="
echo "  PIDs: ${PIDS[*]}"
echo ""
echo "Monitor:"
echo "  uv run python -m scripts.spot_check_rlvr ${LOG_DIR}/* --watch"
echo ""
echo "Kill all:"
echo "  kill ${PIDS[*]}"
echo ""
echo "Tail a run:"
echo "  tail -f ${LOG_DIR}/consult-q3-v1-g8-s42.stdout.log"
echo ""

FAILED=0
for PID in "${PIDS[@]}"; do
  wait "$PID" || FAILED=$((FAILED + 1))
done
if [[ $FAILED -gt 0 ]]; then
  echo "ERROR: ${FAILED}/6 runs failed"
  exit 1
fi
echo "All 6 runs completed successfully."
