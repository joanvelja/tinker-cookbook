#!/bin/bash
# Consultancy experiment v2 — length-normalized advantages
# v2: normalize_advantages_by_length=True, grad_clip=0.3
#
# 3 seeds × G=8 = 3 runs (~$252)
#
# Launch from repo root: bash scripts/launch_consultancy_v2.sh
# Single run:            bash scripts/launch_consultancy_v2.sh 42
#
# Monitor: uv run python -m scripts.spot_check_rlvr logs/consultancy-experiment-v2/* --watch

set -euo pipefail

LOG_DIR="logs/consultancy-experiment-v2"
mkdir -p "$LOG_DIR"
...
run_one() {
  local SEED=$1
  local NAME="consult-v2-g8-s${SEED}"
  local DELAY=$((IDX * 90))
...
echo ""
echo "Tail a run:"
echo "  tail -f ${LOG_DIR}/consult-v2-g8-s42.stdout.log"
echo ""
