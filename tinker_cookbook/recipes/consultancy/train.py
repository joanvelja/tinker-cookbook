"""Consultancy training CLI.

Reference-blind RLVR: a frozen copy of the base model judges responses
without seeing the ground-truth answer. Reuses the RLVR training loop.
"""
...
from tinker_cookbook.recipes.consultancy.builders import DATASET_BUILDER_MAP
...
DatasetChoice = Literal["consultancy_gpqa"]


@chz.chz
class CLIConfig:
    """Command-line configuration for consultancy training."""
    ...
    # Stream minibatch
    stream_groups_per_batch: int | None = None
...
async def cli_main(cli_config: CLIConfig) -> None:
    ...
    run_name = (
        f"consultancy-{cli_config.dataset}-{model_slug}"
        f"-{learning_rate}lr-{datetime.now().strftime('%Y-%m-%d-%H-%M')}"
    )

    log_path = cli_config.log_path or f"/tmp/tinker-examples/consultancy/{run_name}"
