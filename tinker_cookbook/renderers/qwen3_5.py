...
from tinker_cookbook.renderers.qwen3 import Qwen3Renderer, Qwen3VLRenderer
...
class Qwen3_5DisableThinkingRenderer(Qwen3_5Renderer):
    ...
    Matches the Qwen3.5 HF template with enable_thinking=False. The generation
    suffix prefills <think>\\n\\n</think>\\n\\n, signaling the model to respond
    directly without reasoning.

    parse_response prepends the empty think block to the stored content so that
    historical assistant messages reproduce the same token sequence, preserving
    the extension property for KV-cache reuse in multi-turn RL.
    """
    ...
    def build_generation_prompt(
        ...
        """Skip Qwen3Renderer's <think> prefill — _get_generation_suffix handles it."""
        # Bypass Qwen3Renderer.build_generation_prompt which unconditionally adds
        # <think>\n as prefill. Our suffix already contains the full empty think block.
        return super(Qwen3Renderer, self).build_generation_prompt(messages, role, prefill)

    def parse_response(self, response: list[int]) -> tuple[Message, bool]:
        """Prepend empty think block to match generation suffix prefill.

        The generation suffix includes <think>\\n\\n</think>\\n\\n which is not
        part of the sampled tokens. Prepending it to the stored text ensures
        historical assistant messages reproduce the same token sequence,
        preserving the extension property.
        """
        message, success = super().parse_response(response)
        if not success:
            return message, success
        ...
        content = message.get("content")
        if isinstance(content, str) and not content.startswith(self._EMPTY_THINK_PREFIX):
            message["content"] = self._EMPTY_THINK_PREFIX + content
